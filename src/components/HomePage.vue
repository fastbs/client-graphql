<template>
  <div class="card mb-3 flex flex-column justify-content-center" style="padding-top: 1.5rem;">
    ...
    <div id="my-element" class="card flex justify-content-center flex-wrap gap-3 mt-2 mb-2">
      <p>Counter: {{ store.count }} Toasts length: {{ store.toasts.length }} AE length: {{ store.apolloErrors.length }}
      </p>
...
const ci = async () => {
  store.increment();
  const ret = await EmployesService.fetch({ _id: "12345" });
  //const ret = await EmployesService.fetch({ name: "Восьмихуев Евстафий Ануфриевич" });

  //console.log("ret: ", ret);
  //console.log("store.toasts from HomePage: ", store.toasts);
}
...
/*         socket.emit('identity', 555, (response: any) =>
          console.log('*** WS *** Identity:', response),
        );

        socket.emit('fbs', "Sample string", (response: any) =>
